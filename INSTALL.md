...
* `LE_CERT`, `LE_KEY` — Путь к сертификатам Let's Encrypt. Монтируются как отдельные файлы (Docker сам разрешает symlink live→archive). Если пусто — используется self-signed серт, встроенный в образ.
