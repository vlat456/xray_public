...
Скрипт:
1. Сохраняет `.env`, кастомный decoy (`nginx/html/`) и SSL-серты (`ssl/`) в `/tmp/`
2. Делает `git pull` (или `fetch + reset` при конфликтах)
3. Восстанавливает сохранённые пользовательские файлы
4. Пересобирает Docker-образы (`docker compose build --pull`)
5. Перезапускает контейнеры (`docker compose up -d`)
6. Проверяет, что xray и nginx запущены

**Важно:** Если у вас кастомный decoy-сайт — он сохранится.
То же самое с `.env` и сертификатами. Можно запускать без страха всё потерять.

---

## 14. Systemd (автозапуск при перезагрузке)
...
# Обновить стек до последней версии (сохраняет .env, decoy, SSL)
./update.sh
```
...
## 18. Как это работает (архитектура)
...
### 19.1. Alpine `cut` не возвращает пустую строку
...
### 19.3. После обновления LE сертификатов nginx отдаёт старый серт
...
### 19.4. Nginx "host not found" при старте
...
### 19.5. Разделитель клиентов — `;`, а не `,`
...
### 19.6. XHTTP несовместим с `flow: xtls-rprx-vision`
...
### 19.8. `docker compose restart` не применяет новый .env
...
### 19.9. `post_check.sh` HTTPS тест падает с 400
...
### 19.10. fwupd жрёт 210MB RAM на Ubuntu 24.04
...
### 19.11. `list_users.sh` давал всем клиентам одинаковый shortId
